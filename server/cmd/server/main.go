...
func main() {
	...
	api := r.Group(cfg.Server.ContextPath)
	{
		...
		if application.ComicHandler != nil {
			...
			comic.POST("/confirm-storyboard", comicAuth, application.ComicHandler.ConfirmStoryboard)
			comic.POST("/retry", comicAuth, application.ComicHandler.Retry)
			comic.POST("/regenerate-panel", comicAuth, application.ComicHandler.RegeneratePanel)
