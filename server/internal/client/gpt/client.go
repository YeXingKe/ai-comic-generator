...
type Client struct {
	...
	baseURL string       // API 基础 URL（如 https://api.wszhu.top）
	model   string       // 模型名称（gpt-image-2 或 gpt-image-2-4k）
	size    string       // 图片尺寸（1024x1024 等，来自配置）
	quality string       // 图片质量（standard | hd，来自配置）
...
func NewClient(cfg *config.OpenAIImageConfig) (*Client, error) {
...
func (c *Client) GenerateWithSize(ctx context.Context, prompt, destPath, size string) error {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", // 创建带上下文的 POST 请求
		c.baseURL+"/v1/images/generations", // API 端点
