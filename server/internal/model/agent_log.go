...
type AgentLog struct {
	...
	TaskID       string     `gorm:"column:taskId;type:varchar(64);not null;index" json:"taskId"`          // 关联的漫画任务 ID（UUID）
	...
	Status       string     `gorm:"column:status;type:varchar(20);not null" json:"status" enums:"RUNNING,SUCCESS,FAILED" example:"SUCCESS"` // 执行状态：RUNNING 进行中 / SUCCESS 成功 / FAILED 失败
